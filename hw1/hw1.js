...
const Product = class Product {
    ...
    constructor (inputProd){
        let prodArray = inputProd.split(',');
        this.name = prodArray[0];
        this.price = parseFloat(prodArray[1]);
        this.availability = prodArray[2];
    }
    ...
    static inStock = (products) => {
        let inStockProd = products.filter(product => product.availability === "In Stock")
        return inStockProd;
    }
    ...
    static halfOff = (products) => {
        // let halfProd = products.map((product) => new Product('${product.name},${product.price * 2},${product.availability}'));
        let a = 0.5
        let halfProd = products.map((product) => new Product(`${product.name},${product.price * 0.5},${product.availability}`));
        return halfProd;
    }
    ...
    static printProducts = (products) => {
        products.forEach(product => {
            console.log(`Product: ${product.name}, Cost: $${product.price.toFixed(2)}, Availability: ${product.availability === "In Stock"? 'Yes' : 'No'}`)
        });
    }
